...
export function toReport(session: Session): string {
  ...
  const title = escapeHtml(session.title || "Factify session");

  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
<title>${title} — Factify report</title>
...
<body>
<main>
  <div class="eyebrow"><span class="dot"></span>Factify session report</div>
  <h1>${title}</h1>
  <div class="meta">
  ...
  <div class="footer">
    Generated by <a href="https://factify-rose.vercel.app/">Factify</a>
  </div>
</main>
</body>
</html>`;
